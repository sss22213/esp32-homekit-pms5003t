...
HAPError HandleAirQualityPM25Read(
        ...
        /* Receive PM25 every second from sensor */
        if(xQueueReceive(pms5003t_queue, &pms5003t_data_tmp, 1000 / portTICK_RATE_MS) == pdPASS){
            if(pms5003t_data_tmp.PM2_5 <= 1000 && pms5003t_data_tmp.PM2_5 >= 0){
                ...
                /* Get AirQuality over PM25 */
                if(pms5003t_data_tmp.PM2_5 < 15) {
                    airquilitynum = 1;
                } else if(pms5003t_data_tmp.PM2_5 >= 15 && pms5003t_data_tmp.PM2_5 < 35) {
                    airquilitynum = 2;
                } else if(pms5003t_data_tmp.PM2_5 >= 35 && pms5003t_data_tmp.PM2_5 < 54) {
                    airquilitynum = 3;
                } else if(pms5003t_data_tmp.PM2_5 >= 54 && pms5003t_data_tmp.PM2_5 < 150) {
                    airquilitynum = 4;
                } else {
                    airquilitynum = 5;
                }
...
HAPError HandleAirQualityPM10Read(
        ...
        /* Receive PM10 every second from sensor */
        if(xQueueReceive(pms5003t_queue, &pms5003t_data_tmp, 1000 / portTICK_RATE_MS) == pdPASS){
